...
async function validateWeeklyOff({
  ...
  for (const entry of offEntries) {
    ...
    requestWeeks.set(weekKey, entry.leaveDate);
  }
  ...
  for (const entry of offEntries) {
    ...
    if (existingOffCount > 0) {
      errors.push({
        code: 'OFF_ALREADY_EXISTS_IN_WEEK',
        leaveDate: entry.leaveDate,
        message:
...
async function validateOffBalance({
  ...
  const requestedOffCount = entries.filter(
    (entry) => entry.leaveTypeCode === 'OFF'
  ).length;
  ...
  const offBalance = await getOffBalance({
    empId,
    cycleId,
    transaction
  });

  if (requestedOffCount > offBalance.remaining) {
    errors.push({
      code: 'INSUFFICIENT_OFF_BALANCE',
      requested: requestedOffCount,
      remaining: offBalance.remaining,
      message:
...
async function validateOffRequireHC({
  ...
  // Lấy task hiện tại của nhân viên
  const submissionTask =
    await RegistrationSubmission.findOne({
      where: {
        emp_id: empId,
        cycle_id: cycleId
      },
  ...
  if (
    submissionTask &&
    submissionTask.entries &&
    submissionTask.entries.length > 0
  ) {
    taskId =
      submissionTask.entries[0].task_id;
  }

  // Khi chưa có submission cũ thì lấy task từ lịch sử task
  if (!taskId) {
    const [taskRows] =
      await RegistrationSubmission.sequelize.query(
        `
          SELECT task_id
    ...
    return [
      {
        code: 'TASK_NOT_FOUND',
        message:
          'Không tìm thấy task hiện tại của nhân viên.'
      }
    ];
  }
  ...
  for (const entry of offEntries) {
    const requireHC = await RequireHC.findOne({
      where: {
        cycle_id: cycleId,
        working_date: entry.leaveDate,
        task_id: taskId
      },
      transaction
    });
    ...
    if (
      totalAfterRequest >
      Number(requireHC.max_off)
    ) {
      errors.push({
        code: 'REQUIRE_HC_FULL',
        leaveDate: entry.leaveDate,
        maxOff: Number(requireHC.max_off),
        registeredOff: registeredOffCount,
        requestedOff: requestOffCount,
        message:
