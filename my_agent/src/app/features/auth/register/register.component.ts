    ...
    <section class="min-h-screen bg-slate-100 px-4 py-10 text-slate-800">
      <div class="mx-auto flex max-w-md flex-col rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
        <div class="mb-6 text-center">
          <h1 class="text-2xl font-bold text-emerald-700">Đăng ký</h1>
          <p class="mt-2 text-sm text-slate-500">Tạo tài khoản mới cho hệ thống</p>
        </div>

        <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4">
          <div>
            <label class="mb-1 block text-sm font-medium">Mã nhân viên</label>
            <input formControlName="empCode" class="w-full rounded-lg border border-slate-300 px-3 py-2" />
          </div>

          <div>
            <label class="mb-1 block text-sm font-medium">Tên nhân viên</label>
            <input formControlName="empName" class="w-full rounded-lg border border-slate-300 px-3 py-2" />
          </div>
        ...
        <div class="mt-6 text-center text-sm text-slate-500">
          Đã có tài khoản?
          <a routerLink="/login" class="ml-1 font-semibold text-emerald-700">Đăng nhập</a>
        </div>
      </div>
    </section>
  `
